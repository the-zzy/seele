...
| 环境 | 位置 | 访问方式 | 用途 |
|------|------|----------|------|
| 测试环境 | 本地开发机 | `http://localhost:8000` / `http://127.0.0.1:9000` | 开发、本地验证 |
| 正式环境 | 远程服务器 | 域名 / 公网 IP | 生产部署 |

**禁止事项**：
- 禁止把本地测试配置、调试数据、`.env` 上传到远程正式环境
- 禁止把远程正式环境的 IP、路径、凭据写入仓库或共享文档
- 禁止在本地直接连接或操作远程生产数据库（除非明确授权并备份）

**文档敏感信息规范**：
- 仓库内的迁移手册、部署脚本、README 等文档只能包含通用操作模板和占位符（如 `$PROD_SERVER_IP`、`$SSH_KEY_PATH`、`$DB_PASS`）
- 真实服务器 IP、SSH 私钥路径、数据库密码提取命令等敏感信息必须保存在本地环境变量或加密运维文档中，禁止提交到仓库

**本地启动规范**：
- 启动前按「Windows 进程管理」章节彻底清理旧进程
- 前端固定使用 **8000** 端口：`npm run serve`
- 后端固定使用 **9000** 端口：`python start.py`

**正式部署规范**：
- 必须通过 PR 合并到 `main` 后再部署
- 部署脚本和配置文件使用占位符，真实值保存在本地或加密运维文档
